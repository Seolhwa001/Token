...
class PipelineMigration {
  ...
  Future<void> runIfNeeded() async {
    ...
    if ((await ResourceRepository().loadAll()).isNotEmpty ||
        (await TransactionRepository().loadAll()).isNotEmpty) {
      await preferences.setBool(_flag, true);
      return;
    }

    final legacyResourceRaw = preferences.getString('token.resources.v1');
    final legacyTransactionRaw = preferences.getString('token.transactions.v1');
    ...
    final legacyTransactions = legacyTransactionRaw == null || legacyTransactionRaw.isEmpty
        ? <dynamic>[]
    ...
    for (final item in legacyTransactions) {
      ...
      final previous = spendByResource[resourceId] ?? TokenAmount.fromMinorUnits(BigInt.zero);
      spendByResource[resourceId] = previous + tokenAmount;
    }
    ...
    for (final item in legacyResources) {
      ...
      final historicalSpend = spendByResource[id] ?? TokenAmount.fromMinorUnits(BigInt.zero);
    ...
    for (final item in legacyTransactions) {
      ...
      final transaction = TokenTransaction(
        ...
        appliedExchangeRate: ExchangeRate.parse(map['appliedExchangeRate'] as String),
        merchant: '',
      ...
      ledger.add(LedgerEntry(
        ...
        description: transaction.memo.isEmpty ? '기존 거래' : transaction.memo,
        transactionId: id,
        createdAt: createdAt,
      ));
      ...
      for (final state in [
        ...
        lifecycle.add(TransactionLifecycleEvent(
    ...
    await ClassificationRepository().saveAll(classifications);
    await LedgerRepository().saveAll(ledger);
